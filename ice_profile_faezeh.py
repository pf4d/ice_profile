...
du  = TrialFunction(Q)               # trial function for solution
phi = TestFunction(Q)                # test function in mixed space

u.vector().set_local(zeros(num))
...
fu    = + rho_i * g * H * zs.dx(0) * phi * dx \
        + mu * Bs * ((H - rho_p/rho_i * zb) * u)**(1/m) * phi * dx \
